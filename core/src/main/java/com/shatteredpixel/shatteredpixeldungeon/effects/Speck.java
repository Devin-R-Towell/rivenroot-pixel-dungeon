...
public class Speck extends Image {
	...
	public static final int TWINKLE     = 120;  // Added for TWINKLE
	...
	public void reset( int index, float x, float y, int type ) {
		...
		switch (type) {
			...
			case TWINKLE:
				frame( film.get( STAR ) );
				angle = Random.Float( 360 ); // Make base scale smaller
				scale.set(0.5f);  // Start at half size
				lifespan = 1.5f;
				break;
		}
	...
	public void update() {
		...
		if (left <= 0) {
			...
			switch (type) {
				...
				// Added for TWINKLE
				case TWINKLE:
					float pos = left/lifespan;
					// Smaller scale variation
					scale.set(0.3f + 0.2f * (float)Math.sin(left * 4));  // Pulse between 0.3x and 0.5x size
					am = pos < 0.9f ? 1 : (1 - pos) * 10;
					break;
			}
		}
	}
