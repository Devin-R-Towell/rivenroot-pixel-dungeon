...
public class Item implements Bundlable {
	...
	public int targetingPos2(int dst1, int dst2) {
		return throwPos2(dst1, dst2);
	}
	...
	public int throwPos2(int dst1, int dst2) {
		int dst3 = Ballistica.RicochetCalc(dst1, dst2);
		return new Ballistica(dst2, dst3, Ballistica.PROJECTILE).collisionPos;
	}
	...
	public void recast(final int dst1, final int dst2) {
		final int cell3 = throwPos2(dst1, dst2);
		Hero user = Dungeon.hero;
		Char enamy2 = Actor.findChar(cell3);

		user.sprite.zap(cell3);
		user.busy();

		throwSound();

		Char enemy2 = Actor.findChar(cell3);

		final float delay = castDelay(user, dst2);
					...
					reset(user.sprite,
							...
							new Callback() {
								...
								public void call() {
									curUser = user;
									Item i = Item.this;
									if (i != null) i.onThrow(cell3);
									if (curUser.hasTalent(Talent.IMPROVISED_PROJECTILES)
											&& !(Item.this instanceof MissileWeapon)
											&& curUser.buff(Talent.ImprovisedProjectileCooldown.class) == null) {
										if (enemy2 != null && enemy2.alignment != curUser.alignment) {
											Sample.INSTANCE.play(Assets.Sounds.HIT);
											Buff.affect(enemy2, Blindness.class, 1f + curUser.pointsInTalent(Talent.IMPROVISED_PROJECTILES));
											Buff.affect(curUser, Talent.ImprovisedProjectileCooldown.class, 50f);
										}
									}
									if (user.buff(Talent.LethalMomentumTracker.class) != null) {
